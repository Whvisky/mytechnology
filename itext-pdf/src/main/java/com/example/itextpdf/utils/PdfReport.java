...
//            writer.setPageEvent(new MyHeaderFooter());// 页眉/页脚
    ...
    static {
        try {
            ...
            BaseFont bfEn = BaseFont.createFont(BaseFont.HELVETICA, BaseFont.WINANSI,
                    BaseFont.EMBEDDED);
...
//        gotoP.setReference("#top");
        ...
        // 添加图片
        BarcodeQRCode barcodeQRCode = new BarcodeQRCode("https://www.upwhk.com/", 180, 180, null);
        Image image = barcodeQRCode.getImage();
//        image.scalePercent(20f, 20f);
        image.scalePercent(80f); //依照比例缩放
        // 自己表格
        PdfPTable table1 = createTable(new float[] { 100, 240});
        table1.setSpacingBefore(10f); // 表格与上面元素的距离
        table1.addCell(createCell("预约号码"));
        table1.addCell(createCell("A034"));

        table1.addCell(createCell("Exhibition"));
        table1.addCell(createCell("SZ Fashion source2"));

        table1.addCell(createCell("Stand Number"));
        table1.addCell(createCell("F02"));

        table1.addCell(createCell("Venue"));
        table1.addCell(createCell("National Exhibition and Convention Center\n" +
                "(Shanghai) South Entrance, 168 East Yingguan Road,\n" +
                "Shanghai, China\n"));

        table1.addCell(createCell("Date"));
        table1.addCell(createCell("10/12/2017"));

        table1.addCell(createCell("Party size"));
        table1.addCell(createCell("3"));

        table1.addCell(createCell("Company name"));
        table1.addCell(createCell("aaa"));

        table1.addCell(createCell("Company address"));
        table1.addCell(createCell("ddd dongguan Guangdong China"));

        table1.addCell(createCell("Contact"));
        table1.addCell(createCell(""));

        table1.addCell(createCell("Telephone"));
        table1.addCell(createCell("076922262232"));

        table1.addCell(createCell("Sales region"));
        table1.addCell(createCell("China"));

        table1.addCell(createCell("Preferred Sales"));
        table1.addCell(createCell("SERENA CHUI"));

        table1.addCell(createCell("Language preference"));
        table1.addCell(createCell("Chinese"));

        table1.addCell(createCell("Products of interests"));
        table1.addCell(createCell("Cashmere"));

//        for (int i = 0; i < 5; i++) {
//            float[] borderWidth = new float[]{0,0,0,0};
//            table1.addCell(createCell("起床", textfont, Element.ALIGN_LEFT, borderWidth, borderWidth,false));
//            table1.addCell(createCell("吃午饭", textfont, Element.ALIGN_LEFT, borderWidth, borderWidth,false));
//        }
//        table1.addCell(createCell("Reference Number", keyfont, Element.ALIGN_LEFT));
//        table1.addCell(createCell("Reference Number", keyfont, Element.ALIGN_LEFT));
